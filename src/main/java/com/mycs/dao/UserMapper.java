...
public interface UserMapper {
	...
	@Insert("insert into users(name,sex,addr,password,birth,head,gre) value(#{name},#{sex},#{addr},#{password},#{birth},#{head},#{gre})")
	public void add(User user);

	@Select("select * from users where id = #{id}")
	 public User queryid(int id);

	 @Update("update users set  name=#{name},sex=#{sex},addr=#{addr},password=#{password},birth=#{birth},head=#{head},gre=#{gre} 	where id=#{id}")
